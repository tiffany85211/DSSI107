...
# url2017 <- 'https://github.com/bpb27/trump_tweet_data_archive/raw/master/condensed_2017.json.zip'
# res <- GET(url2017, write_disk("tweets/condensed_2017.json.zip", overwrite = T))

all <- readRDS("trumptweets.rds")
cols <- c("text", "id", "retweeted", "favorited", "favorite_count", "in_reply_to_screen_name", "created_at", "in_reply_to_status_id", "in_reply_to_user_id", "source")

tweets <- do.call("rbind", lapply(all, function(x){x[, cols]}))
