...
```{r}
path <- '//*[@id="main-container"]//div[contains(@class, "title")]/a'
...
```{r}
links <- read_html(url) %>%
    html_nodes("div.r-ent div.title a") %>%
    html_attr("href") %>%
...
link <- all_links[1]
doc <- read_html(link)
meta.css <- "div.article-metaline span.article-meta-value"
metadata <- html_text(html_nodes(doc, meta.css))
    
post.xpath <- '//*[@id="main-content"]/text()' # text() to retrieve untagged text part only
post.paragraph <- html_text(html_nodes(doc, xpath = post.xpath))
print(length(post.paragraph))
